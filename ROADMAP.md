...
**Companion release: FIRST RESIDENTS — 30 FREE NFT COLLECTIBLES.** Fifteen flies
and fifteen frogs, free mint with network gas paid by the minter, on Robinhood
Chain. Original on-chain artwork, one mint per address. The gallery and contract
are being prepared; a live mint address will be listed after verified deployment.
This promotional collection is separate from the fungible token milestone.
[Collection contract and launch checklist](specs/19-residents-nft.md).
...
watchlist, observed token-flow clusters and user-labelled smart-wallet targets.
Provide a separate deterministic demo with immediate virtual copy entries and
bounded exposure. Read live Robinhood Chain transfer evidence without inventing
swap directions or profitable-wallet rankings. Real automatic execution needs
verified swap attribution, a fast provider, signer setup and explicit limits.

Implemented locally: explicit SPIDER entry, eight articulated legs, demo copy
ledger and public-RPC transfer reader. Desktop/mobile fixture checks pass for
motion, pause, reduced motion, selection, watchlists, errors and cleanup.
...
bold angular silhouettes, expressive faces and circular frames. Use five
expressions and three accessories per species, with matching square PFP crops.

Use the existing warm black, olive, lime and cream palette with monospace
typography for the collection, bot wardrobe, frog radar and local release page.
Recolor all 30 original NFT portraits and regenerate their on-chain metadata,
gallery and PFP crops together. Reuse shared UI tokens for future surfaces.
Owners: specs 18–20. Implemented locally, including shared UI tokens and
...
wallet ownership before equipping; retain local profile choices and recheck
ownership after reload or transfer. Skins do not affect trading parameters.

Plan a separate proof-of-work NFT series inspired by browser mining: explicit
Start/Stop, CPU workers and optional WebGPU, visible measured hash rate, best
candidate and an on-chain-verifiable claim. Supply, difficulty, issuance rate
and artwork rules will be specified before implementing its miner or contract.
The original 30 remain free direct mints, with only network gas paid by users.

Both collections should support standard ERC-721 transfers and listing for
resale on OpenSea after deployment and indexing. OpenSea announced Robinhood
Chain NFT support on [2026-07-11](https://opensea.io/blog/articles/robinhood-chain-is-live-on-opensea).
Collection discovery, metadata rendering and an actual listing flow remain
release acceptance checks; a buyer or sale price is not guaranteed.

Implemented locally: First Residents bot profiles, wallet-owned skin selection,
local persistence, reload verification, reset and transfer revocation. Desktop
and mobile browser checks pass with an isolated wallet/RPC fixture; 287 tests
...
zero; the minter pays network gas. Enforce the supply cap and one lifetime mint
per address in an ERC-721 contract. Include original on-chain SVG/JSON metadata,
a 30-piece gallery, explicit wallet/network selection and receipt-confirmed mint
status. This collectible release is separate from the planned fungible token.

Implemented locally: all 30 SVG/JSON previews are generated by the contract;
287 tests / 20 files, typecheck and production build pass. Desktop/mobile gallery
and wallet/RPC fixture checks pass. An unsigned mainnet creation estimate and a
...
breakouts and top-five book imbalance. Show measured triggers, current contacts
and explicit market provenance in a responsive animated pond. Observe existing
snapshots only; do not change agents, trades, the terminal or the market feed.

Implemented locally: 263 tests / 17 files, typecheck, production build and
desktop/mobile browser interaction checks pass. Publication remains separate.
